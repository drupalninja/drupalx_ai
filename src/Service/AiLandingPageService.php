...
final class AiLandingPageService {
   ...
   * @param array|null $allowedParagraphTypes
   *   Optional array of allowed paragraph types. If not provided, will be fetched.
   *
   * @return string|null
  ...
  public function createLandingNodeWithAiContent(string $page_title, array $paragraphs, ?array $allowedParagraphTypes = NULL): ?string {
    try {
      ...
      // Filter out unallowed paragraph types.
      $validParagraphs = [];
      foreach ($paragraphs as $index => $paragraphData) {
        ...
        $validParagraphs[] = $paragraphData;
      }

      if (empty($validParagraphs)) {
        $this->loggerFactory->get('drupalx_ai')->error('No valid paragraphs found after filtering. Allowed types: @allowed', [
          '@allowed' => implode(', ', $allowedParagraphTypes),
        ]);
        return NULL;
      }
